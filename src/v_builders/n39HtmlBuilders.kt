...
fun renderProductTable(): String {
    return html {
        table {
            ...
            getProducts().forEachIndexed { index, it ->
                tr {
                    td(getCellColor(index, 0)) { text(it.description) }
                    td(getCellColor(index, 1)) { text(it.price) }
                    td(getCellColor(index, 2)) { text(it.popularity) }
                }
            }
        }
    }.toString()
